...
function createPagination(currentPage, totalPages, onPageChange) {
    ...
    // 이전 버튼
    if (currentPage > 1) {
        html += `<button class="btn-sm btn-secondary" onclick="${onPageChange}(${currentPage - 1})">이전</button>`;
    }
    
    // 페이지 번호
    for (let i = 1; i <= totalPages; i++) {
        if (i === currentPage) {
            html += `<button class="btn-sm btn-primary">${i}</button>`;
        } else {
            html += `<button class="btn-sm btn-secondary" onclick="${onPageChange}(${i})">${i}</button>`;
        }
    }
    
    // 다음 버튼
    if (currentPage < totalPages) {
        html += `<button class="btn-sm btn-secondary" onclick="${onPageChange}(${currentPage + 1})">다음</button>`;
    }
...
function renderTable(columns, data, options = {}) {
    ...
    html += '<tbody>';
    if (data.length === 0) {
        html += `<tr><td colspan="${columns.length}" class="text-center text-gray-500">데이터가 없습니다</td></tr>`;
    } else {
        data.forEach(row => {
            html += '<tr>';
            columns.forEach(col => {
