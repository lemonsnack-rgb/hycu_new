...
function renderThesisInfo(assignment) {
    ...
    return `
        <div class="bg-white rounded-lg shadow-md p-6 mb-6">
            ...
            <div class="bg-gray-50 rounded-lg p-4 mb-4">
                ...
                <div class="space-y-3">
                    <div class="info-row">
                        ...
                        <div class="info-value">${assignment.studentName} (${assignment.studentNumber})</div>
                    </div>
                    <div class="info-row">
                        <div class="info-label">전공 / 학위과정</div>
                        <div class="info-value">${assignment.major} / ${assignment.degree || '석사'}</div>
                    </div>
                    <div class="info-row">
            ...
            <div class="bg-gray-50 rounded-lg p-4 mb-4">
                <h4 class="font-bold text-gray-800 mb-3">논문 내용</h4>
                <div class="info-row">
            ...
            <div class="bg-gray-50 rounded-lg p-4 mb-4 border border-gray-300">
                <h4 class="font-bold text-gray-800 mb-4">표절/AI 검사 결과</h4>
                <div class="bg-white rounded-lg p-4 text-center border border-gray-300">
                    <div class="text-2xl font-bold text-gray-800">
                        <span class="${copyKiller <= 10 ? 'text-green-600' : copyKiller <= 20 ? 'text-yellow-600' : 'text-red-600'}">CopyKiller ${copyKiller}%</span>
                        <span class="text-gray-400 mx-3">/</span>
                        <span class="${gptKiller <= 10 ? 'text-green-600' : gptKiller <= 20 ? 'text-yellow-600' : 'text-red-600'}">GPT킬러 ${gptKiller}%</span>
                    </div>
                </div>
                <div class="mt-4">
                    <button onclick="window.open('#', '_blank')" 
                            class="w-full bg-white hover:bg-gray-50 text-gray-800 font-medium py-2 px-4 border border-gray-300 rounded-lg">
                        검사 결과 보고서
                    </button>
                </div>
            </div>
    ...
    if (evaluation.evaluationType === 'passfail') {
        return `
            <div class="bg-white rounded-lg shadow-md p-6 mb-6">
                ...
                <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                    <p class="text-sm text-blue-800">
                        <strong>제출일시:</strong> ${formatDateTime(evaluation.submittedAt)}
                    </p>
                </div>
                
                <div class="space-y-3">
                    ${evaluation.passFailResults.map((result, index) => {
                        const isPassed = result.result === 'pass';
                        return `
                            <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                                <div class="flex items-start justify-between mb-2">
                                    <div class="flex-1">
                                        <h4 class="font-bold text-gray-800">${index + 1}. ${result.itemName || template.items[index]?.name}</h4>
                                    </div>
                                    <div>
                                        <span class="px-3 py-1 rounded-full text-sm font-bold ${isPassed ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}">
                                            ${isPassed ? 'Pass' : 'Fail'}
                                        </span>
                                    </div>
                                </div>
                ...
                <div class="mt-6 ${evaluation.passed ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'} rounded-lg p-6 border-2">
                    <div class="flex items-center justify-between">
                        <div>
                            <h4 class="text-lg font-bold text-gray-800">최종 결과</h4>
                            <p class="text-sm text-gray-600 mt-1">Pass: ${evaluation.passCount}, Fail: ${evaluation.totalCount - evaluation.passCount}</p>
                        </div>
                        <div class="text-3xl font-bold ${evaluation.passed ? 'text-green-600' : 'text-red-600'}">
                ...
                ${evaluation.overallComment ? `
                    <div class="mt-6 bg-gray-50 rounded-lg p-4">
                        ...
                        <div class="text-sm text-gray-600 bg-white rounded p-3">${evaluation.overallComment}</div>
                    </div>
                ` : ''}
            </div>
        `;
        ...
        <div class="bg-white rounded-lg shadow-md p-6 mb-6">
            ...
            <div class="space-y-4">
                ${evaluation.scores.map((score, index) => {
                    const category = template.categories.find(c => c.id === score.categoryId);
                    if (!category) return '';
                    ...
                    return `
                        <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                            <div class="flex items-start justify-between mb-2">
                                <div class="flex-1">
                                    <h4 class="font-bold text-gray-800">${index + 1}. ${score.categoryName || category.name}</h4>
                                    ...
                                    ${category.criteria && category.criteria.length > 0 ? `
                                        <ul class="text-xs text-gray-500 mt-2 ml-4 space-y-1">
                                            ${category.criteria.map(c => `<li>• ${c}</li>`).join('')}
                                        </ul>
                                    ` : ''}
                                </div>
                                <div class="text-right ml-4">
                                    <div class="text-lg font-bold text-blue-600">${score.score}점</div>
                                    <div class="text-xs text-gray-500">가중치: ${score.weight}%</div>
                                </div>
                            </div>
                            ${score.comment ? `
                                <div class="mt-2 text-sm text-gray-600 bg-white rounded p-2">
            ...
            <div class="mt-6 bg-blue-50 rounded-lg p-6 border-2 border-blue-200">
                <div class="flex items-center justify-between">
                    <div>
                        <h4 class="text-lg font-bold text-gray-800">총점</h4>
                    </div>
                    <div class="text-4xl font-bold text-blue-600">${evaluation.totalScore}점</div>
                </div>
            </div>
            ...
            ${evaluation.overallComment ? `
                <div class="mt-6 bg-gray-50 rounded-lg p-4">
                    <h4 class="font-bold text-gray-800 mb-3">종합 의견</h4>
                    <div class="space-y-3">
                        ${evaluation.overallComment.strengths ? `
                            <div>
                                <div class="text-sm font-medium text-gray-700 mb-1">장점:</div>
                                <div class="text-sm text-gray-600 bg-white rounded p-3">${evaluation.overallComment.strengths}</div>
                            </div>
                        ` : ''}
                        ${evaluation.overallComment.improvements ? `
                            <div>
                                <div class="text-sm font-medium text-gray-700 mb-1">개선사항:</div>
                                <div class="text-sm text-gray-600 bg-white rounded p-3">${evaluation.overallComment.improvements}</div>
                            </div>
                        ` : ''}
                        ${evaluation.overallComment.conclusion ? `
                            <div>
                                <div class="text-sm font-medium text-gray-700 mb-1">결론:</div>
                                <div class="text-sm text-gray-600 bg-white rounded p-3">${evaluation.overallComment.conclusion}</div>
                            </div>
                        ` : ''}
                    </div>
                </div>
...
function renderChairSummary(detail) {
    ...
    return `
        <div class="bg-white rounded-lg shadow-md p-6 mb-6">
            ...
            <!-- ID 52, 53: 심사위원별 상세 점수 표시 -->
            <div class="bg-gray-50 rounded-lg p-4 mb-6 border border-gray-300">
                <h4 class="font-bold text-gray-800 mb-4">심사위원 평가 현황</h4>
                <div class="space-y-4">
                    ${evaluations.map(eval => `
                        <div class="bg-white rounded-lg border border-gray-300 p-4">
                            <div class="flex items-center justify-between mb-3 pb-3 border-b">
                                <div>
                                    <p class="font-bold text-gray-800">${eval.professorName}</p>
                                    <p class="text-sm text-gray-600">${eval.role === 'chair' ? '심사위원장' : '심사위원'}</p>
                                </div>
                                <div class="text-right">
                                    <p class="text-xs text-gray-500">총점</p>
                                    <p class="text-2xl font-bold text-blue-600">${eval.totalScore}점</p>
                                </div>
                            </div>
                            
                            <!-- 항목별 점수 -->
                            <div class="space-y-2">
                                ${(eval.scores || []).map(score => `
                                    <div class="flex items-center justify-between text-sm">
                                        <span class="text-gray-600">${score.categoryName || score.categoryId || '평가항목'}</span>
                                        <span class="font-semibold text-gray-800">${score.score}점 <span class="text-xs text-gray-400">(${score.weight}%)</span></span>
                                    </div>
                                `).join('')}
                            </div>
                            
                            <!-- 종합 의견 -->
                            ${eval.overallComment?.combined ? `
                                <div class="mt-3 pt-3 border-t">
                                    <p class="text-xs text-gray-500 mb-1">종합 의견</p>
                                    <p class="text-sm text-gray-700">${eval.overallComment.combined}</p>
                                </div>
                            ` : ''}
                        </div>
                    `).join('')}
