...
### 11.1 지도교수 배정 (Advisor Assignment)

**파일:** `admin-v3/assets/js/admin_views.js`
**적용 라인:** 2770-2884

#### 화면 구성
- **타이틀 영역**: 없음 (검색 영역부터 바로 시작)
- **검색 필드**: 12개 (3행 × 5열 그리드 - 마지막 행은 1개 필드 + 버튼)
- **테이블**: 학생 목록 (연구계획서 기반)
...
#### 특징
- **타이틀 제거**: 페이지 상단 타이틀 영역 없이 검색부터 시작
- **배정상태 필터**: 배정완료/미배정 구분 검색
- **지도교수명 검색**: 주지도교수 이름으로 검색 가능
- **Mock 데이터 대응**: 일부 필드는 Mock 데이터에 없어 주석 처리 (학기, 대학구분, 계열/대학원, 학부(과)전공)

#### 검색 함수
**파일:** `admin-v3/assets/js/admin_main.js` (라인 3335-3424)

**주요 기능:**
```javascript
function searchAdvisorAssignment() {
    // 12개 필드 값 수집
    // 학위과정, 학적상태, 배정상태, 지도교수명 필터링
    // Mock 데이터 없는 필드는 주석 처리
    // showNotification으로 결과 표시
}
```
...
| 1.1 | 2026-01-05 | 지도교수 배정 화면 사례 추가 (타이틀 제거 반영) |
...
2. **사용자 승인 대기**
   - 사용자가 변경 사항을 검토하고 승인할 때까지 대기
   - 승인 없이 `git add`, `git commit`, `git push` 실행 금지

3. **승인 후 실행**
   - 사용자 승인이 확인된 후에만 git 명령어 실행
   - 커밋 메시지는 명확하고 상세하게 작성
   - 푸시 완료 후 커밋 해시와 변경 요약 보고

### 13.2 예외 상황
승인 없이 커밋/푸시할 수 있는 경우는 **없음**. 긴급한 상황에서도 반드시 승인 필요.
...
**문서 작성 기준 화면:** 학기별 지도 계획 (Weekly Guidance), 지도교수 배정 (Advisor Assignment)
**최종 검토일:** 2026-01-05
