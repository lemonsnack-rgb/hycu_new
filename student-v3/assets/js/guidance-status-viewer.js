...
function renderGeneralThread(feedbackId){
  ...
  if (items.length === 0) {
    ...
    // Students cannot add general evaluation
  ...
  listEl.innerHTML = `
    ...
    <div class="general-replies mt-3 ml-4 pl-4 border-l-2 border-gray-300">
      ...
      <!-- Students cannot reply to evaluation comments -->
    </div>
  `;
}
...
window.uploadEditAttachmentGeneral = function(){
  ...
  if (!inp){
    ...
    inp.addEventListener('change', ()=>{
      ...
      console.log('[uploadEditAttachmentGeneral] window._editAttach 설정됨:', window._editAttach);
...
function uploadAttachmentForGeneral(){
  ...
  if (!inp){
    ...
    inp.addEventListener('change', ()=>{
      ...
      console.log('[uploadAttachmentForGeneral] window._pendingAttach 설정됨:', window._pendingAttach);
...
function showGeneralAttachPreview(file) {
    ...
    if (!previewContainer || !filenameEl || !filesizeEl) {
        console.log('[showGeneralAttachPreview] 요소를 찾을 수 없음');
        return;
    }
...
function showEditAttachPreview(file) {
    console.log('[showEditAttachPreview] 함수 호출, file=', file.name);
    ...
    console.log('[showEditAttachPreview] previewContainer=', previewContainer);
    console.log('[showEditAttachPreview] filenameEl=', filenameEl);
    console.log('[showEditAttachPreview] filesizeEl=', filesizeEl);

    if (!previewContainer || !filenameEl || !filesizeEl) {
        console.error('[showEditAttachPreview] ❌ 필수 요소를 찾을 수 없음');
        // 모든 general-edit-attach로 시작하는 요소 찾기
        console.log('[showEditAttachPreview] 모든 관련 요소:',
            Array.from(document.querySelectorAll('[id*="general-edit-attach"]')).map(el => ({id: el.id, display: el.style.display}))
        );
        return;
    }
...
console.log('✅ guidance-status-viewer.js 로드 완료');
