...
const CompleteMeetingModal = {
        ...
        return ModalBase.renderContainer('complete-meeting-modal', `
            ...
            <div class="px-6 py-4">
                <div class="grid grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">
                            실제 소요 시간 (분) <span class="text-red-600">*</span>
                        </label>
                        <input type="number" id="complete-duration"
                ...
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 mb-2">
                        미팅 요약 <span class="text-red-600">*</span>
                    </label>
                    <textarea id="complete-summary"
                              rows="5"
                              placeholder="미팅 내용을 요약하여 입력하세요."
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#6A0028]"></textarea>
                </div>
                ...
                ${meeting.meetingType === 'online' ? `
                    <div class="mb-4">
                        <label class="flex items-center cursor-pointer">
                            <input type="checkbox" id="check-recording" checked class="mr-2">
                            <span class="text-sm text-gray-700">Zoom 녹화 확인</span>
                        </label>
                        <p class="text-xs text-gray-500 mt-1">
                            체크 시 Zoom Cloud Recording을 확인합니다.
                        </p>
                    </div>
                ` : ''}
            </div>
    ...
    confirm() {
        ...
        const actualDuration = parseInt(document.getElementById('complete-duration').value);
        const meetingSummary = document.getElementById('complete-summary').value.trim();
        const checkRecording = document.getElementById('check-recording')?.checked || false;

        // 검증
        const validation = ValidationUtils.validateRequired({
            '완료 날짜': completedDate,
            '실제 소요 시간': actualDuration.toString(),
            '미팅 요약': meetingSummary
        });
