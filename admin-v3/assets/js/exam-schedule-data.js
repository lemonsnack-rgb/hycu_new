...
function getExamScheduleListData() {
    ...
    return completedAssignments.map(assignment => {
        ...
        return {
            ...
            year: assignment.year || '2025',
            semester: assignment.semester || '1',
            graduate: assignment.graduate || '일반대학원', // 대학구분
            college: assignment.college || (
                assignment.department.includes('경영') ? '인문사회계열' :
                assignment.department.includes('컴퓨터') ? '공학계열' :
                assignment.department.includes('교육') ? '인문사회계열' : '공학계열'
            ), // 계열/대학원
            undergraduate: assignment.undergraduate || (
                assignment.department.includes('경영') ? '경영대학' :
                assignment.department.includes('컴퓨터') ? '공과대학' :
                assignment.department.includes('교육') ? '사범대학' : '공과대학'
            ), // 학부(과)전공
            academicStatus: assignment.academicStatus || '재학', // 학적상태
            advisorName: assignment.advisorName || assignment.chairName || '-', // 지도교수명
